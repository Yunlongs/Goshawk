...
def walking_compile_database(file, flag, plugin_dir=config.plugin_dir, temp_dir=config.temp_dir):
...
def plugin_run(project_dir, flag):
    ...
    if not os.path.exists(compile_database_file):
        ...
        exit(-1)
    walking_compile_database(compile_database_file, flag)
