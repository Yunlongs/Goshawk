...
def count_free_call_site(candidate_functions, call_graph):
    ...
    with open("temp/Dealloc_Number.txt","w") as f:
        for funcname, freq in sorted_func_freq:
            line = funcname.ljust(50," ") + str(freq)
            f.write(line + "\n")
    with open("temp/candidate_free.txt","w") as f:
        for funcname, freq in sorted_func_freq:
            f.write(funcname + "\n")
    return func_freq
...
def special_deallocator_identification(call_graph, func_similarity, free_check_file, min_reassignment=10):
    """
    根据通过语义识别出来的释放函数和其被调用次数，寻找其中的每个项目中独自实现的原语deallocator。
    规则：
    1. 返回值类型为void
...
def generate_seed_allocators(belief_function):
    with open(config.seed_alloc_path, "w") as f_w:
        f_w.write("\n".join(belief_function) + "\n")
        with open("subword_dataset/official_allocator.txt", "w") as f_r:
            f_w.write(f_r.read())
...
def run_free(call_graph_file, step=1):
    ...
    if step == 2:
        ...
        sp_funcs = special_deallocator_identification(call_graph, func_similarity, free_check_file, config.min_reset)
...
def initial_candidate_alloc_functions(in_file=config.call_graph_path ,step=1):
    """
    ...
    get_candidate_alloc_function_set(func_similarity, call_graph)

    # step1 : Find the initial strong belief allocation functions
    #belief_functions = initial_special_alloc_function(func_similarity, call_graph)
    #generate_seed_allocators(belief_functions)
